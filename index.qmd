...
::::{.columns} 
::: {.column width=40%}
![](img/headshot.jpg){width="490" height="550"}
:::
::: {.column width=60%}
:mortar_board: Obtained a PhD in Applied Mathematics and Statistics from Newcastle University 

:computer: Worked as a Data Scientist at the [the National Innovation Centre for Data](https://www.nicd.org.uk/)
...
## :wave: About me: Dr Rob Geada

::::{.columns} 
::: {.column width=40%}
![](img/rob_profile.png){width="520" height="550"}
:::
::: {.column width=60%}
:mortar_board: Obtained a PhD in Computer Science from Newcastle University 
...
::::{.columns} 
::: {.column width=40%}
![](img/merged_pic.png){width="520" height="550"}
:::
::: {.column width=60%}
...
- __inspect and act__: block, redact, or let it through
- __auditable__: every decision is logged, explainable, and deterministic for a given policy
...
| **Accuracy** | Exact for known patterns, brittle for nuance | Good for trained risks, needs data | Flexible, handles novel policies, but can hallucinate |
:::
...
::: {.fragment .fade-in-then-semi-out}
**This is defense in depth applied to AI safety.**
:::
...
::: {.fragment .fade-in-then-semi-out .shrink}
:lock: **Vendor lock-in.** Your safety layer is tied to one provider. 
:::
...
NeMo ships with a catalogue of pre-built rails — activate them in `config.yml`, no custom code needed.
...
| `self_check` | LLM-as-judge | Prompt the LLM to evaluate its own input/output against your policy |
:::
...
::: {.fragment .fade-in-then-semi-out}
- **plain language** — a compliance officer could read this
- **domain-specific** — change to whatever your policy requires
- **input and output are separate** — different rules for what users can ask vs. what the model can say
:::
...
- `/v1/chat/completions` — **drop-in proxy** for your model endpoint. Point your app here instead of the LLM directly — requests are inspected, guarded, and forwarded.

- `/v1/checks` — **validation only**. Send content, get pass/fail — no generation. This is the endpoint that unlocks **gateway-level guardrails**: an AI proxy like [Praxis](https://github.com/praxis-proxy/praxis) can inspect request/response bodies at the network layer and route them to `/v1/guardrail/checks` for policy enforcement — per user, per tool, per MCP server — without modifying application code.
...
flowchart LR
    ...
    subgraph S4["⚙️ TrustyAI Operator on Openshift"]
        direction TB
...
|---|---|---|
| TLS termination | manually configure Ingress/Route + certs | automatic Route with edge (or reencrypt when auth enabled) |
...
::: {.fragment .fade-in-then-semi-out}
Application logs won't answer these. You need **traces** — structured, per-request, per-detector visibility.
:::
...
::: {.fragment .fade-in-then-semi-out}
🔶 **SPO + Translation**: exploits English-biased safety filters: *prompt translated to e.g. Mandarin*
:::
...
## :busts_in_silhouette: Why this matters at scale

- :shield: **Consistency** — every AI app gets the same baseline protections on day one
- :rocket: **Velocity** — teams add domain-specific rails, not boilerplate
- :mag: **Auditability** — OTel traces + MLflow experiments = evidence for compliance
- :arrows_counterclockwise: **Feedback loop** — red teaming results drive platform-wide policy updates

The platform sets the floor. Teams build on it. Nobody starts from zero.
...
:::{.fragment .fade-in-then-semi-out}
![](img/maturity.png){fig-align="center" height=400}
:::
